...
local function AntiGiornoFlowers(Value) end
    ...
    local function StreamerModeCallback()
        ...
        if not Toggles.StreamerMode.Value then
            if isfile("renzhub/streamermode.txt") then
                delfile("renzhub/streamermode.txt");
            end;
            return;
        end;
        if not isfile("renzhub/streamermode.txt") then
            writefile("renzhub/streamermode.txt", "streamer mode lalalallalala");
        end;
...
pcall(function()
    ...
    local function Notify(x, y, Alt)
        ...
        if not Alt then
            Library:Notify(string.format("%s | %s", x or '', y or '') .. ' | Renz Hub', 5)
        else
            Library:Notify(x);
        end;
    end;
    ...
    local Window = Library:CreateWindow({
        Title = 'Renz Hub | Anime Battle Arena',
        Center = true,
    ...
    Notify('Welcome to Renz Hub.', nil, nil);
    Notify('Developed for Renz Hub.', nil, nil);
    ...
    local function GetMacroFiles()
        if isfolder('renzhub/ABA/Macros') then
            return listfiles('renzhub/ABA/Macros') or {1}
        end
        return {}
    end

    local function WriteMacroFile()
        local DirPath = 'renzhub/ABA/Macros/'
    ...
    ThemeManager:SetFolder('renzhub')
    SaveManager:SetFolder('renzhub/ABA')
    if not isfolder('renzhub/ABA/Macros') then makefolder('renzhub/ABA/Macros') end
